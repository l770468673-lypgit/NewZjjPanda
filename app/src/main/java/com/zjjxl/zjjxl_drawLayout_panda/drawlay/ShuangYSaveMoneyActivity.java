...
import com.zjjxl.zjjxl_drawLayout_panda.adapters.GridViewAdapter;
import com.zjjxl.zjjxl_drawLayout_panda.beans.Bean_BaseCard;
import com.zjjxl.zjjxl_drawLayout_panda.beans.Bean_zjjCreateOrder;
import com.zjjxl.zjjxl_drawLayout_panda.beans.CardCharge_bean;
import com.zjjxl.zjjxl_drawLayout_panda.beans.CardRentApply_Bean;
import com.zjjxl.zjjxl_drawLayout_panda.beans.ChargeMoneyRecordbean;
import com.zjjxl.zjjxl_drawLayout_panda.beans.Gson_AlipayRes;
import com.zjjxl.zjjxl_drawLayout_panda.beans.PramDetal_Bean;
...
//    private String mTrace_id;
    ...
    private LinearLayout mLly_alipayfaild;
    private ChargeMoneyRecordbean.DataBean mChargeMoneyRecordbean;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mLly_alipayfaild = findViewById(R.id.lly_alipayfaild);
        ...
        initChangeMoneyRecordDate();

    }

    private void initChangeMoneyRecordDate() {
        //  初始化 充值记录传递的数据
        Bundle extras = getIntent().getExtras();
        if (extras != null) {
            mChargeMoneyRecordbean = (ChargeMoneyRecordbean.DataBean) extras.getSerializable("ChargeMoneyRecordbean");
                ...
                case 0:
                    ...
                    if (data != null) {
                        ...
                        if (mMCardid != null) {
                            ...
                            if (mChargeMoneyRecordbean != null) {
                                money = mChargeMoneyRecordbean.getTradeAmount();
                                mPay_time = mChargeMoneyRecordbean.getPayTime();
                                mOut_trade_no = mChargeMoneyRecordbean.getOrderCode();
                                state = 1;
...
//        ToastUtils.showToast(ShuangYSaveMoneyActivity.this, "-充值提交-" + flag);
        String writestatus;
        ...
        LUtils.d(TAG,"writeSuccess==="+writeSuccess);
        ...
        bean_baseCardCall.enqueue(new Callback<Bean_BaseCard>() {
            ...
            public void onResponse(Call<Bean_BaseCard> call, Response<Bean_BaseCard> response) {
                ...
                if (writeSuccess) {
                    ...
                    // 圈存成功后， 更新 虫子记录的圈存状态
                    UpdateCircleState(mOut_trade_no, 1);
                } else {
                    UpdateCircleState(mOut_trade_no, 2);
                }
            }
    ...
    private void UpdateCircleState(String out_trade_no, int i) {
        Call<ChargeMoneyRecordbean> chargeMoneyRecordbeanCall =
                HttpManager.getInstance().getHttpClient3().rechargeRecordUpdatecircleState(out_trade_no, i);
            ...
            public void onResponse(Call<Bean_BaseCard> call, Response<Bean_BaseCard> response) {
                if (response.body() != null) {
                    ...
                    if (status) {
                        ...
                        rechargeRecordUpdatestate(mOut_trade_no, 6);//订单状态更新
                    } else {
                        ToastUtils.showToast(ShuangYSaveMoneyActivity.this, response.body().getData().getMessage());
                    }
                }
    ...
    private void rechargeRecordUpdatestate(String out_trade_no, int num) {
        Call<ChargeMoneyRecordbean> chargeMoneyRecordbeanCall =
                HttpManager.getInstance().getHttpClient3().rechargeRecordUpdatestate(out_trade_no, num);
        chargeMoneyRecordbeanCall.enqueue(new Callback<ChargeMoneyRecordbean>() {
            @Override
            public void onResponse(Call<ChargeMoneyRecordbean> call, Response<ChargeMoneyRecordbean> response) {
                if (response.body() != null) {
        ...
        if (mParcelableExtra != null) {
            ...
            if (mData != null) {
                if (mData.isOpen.equals(STATE_OPEN)) {
                    tosaveMoneyCommit();
//                    ToastUtils.showToast(this, "卡片已激活");
                    LUtils.d(TAG, "卡片已激活");
                } else {
//               提示开卡
                    LUtils.d(TAG, "卡片未激活");
...
//        mIssuer_code = paramListBean.getIssuer_code();//发卡机构标识//
                    ...
                    if (card_mac != null) {
                        ...
                        try {
                            ...
                            if (mtag != null) {
                                ...
                                refund();
                                // TODO: 2020-09-18   圈存失败， 显示 充值失败按钮， 退款
                                mLly_alipayfaild.setVisibility(View.VISIBLE);
                                mLly_alipaysuccess.setVisibility(View.GONE);
    ...
    private void OpencardApply() {
        ...
        //     ToastUtils.showToast(ShuangYSaveMoneyActivity.this, "-开卡申请-");
                ...
                if (response.body().isStatus()) {
                    ...
                    if (mode_command != null) {
                        ...
                        if (success) {
                            ...
                            refund();
                            ToastUtils.showToast(ShuangYSaveMoneyActivity.this,
                                    "开卡失败" + response.body().getData().getMessage());
                        }
    ...
    //开卡提交
    private void OpencardCommit(int accountNo, String terminalSeq, String server_time,
                                String mode_version, String status) {

        LUtils.d(TAG, "---------OpencardCommit------" + status);
//        ToastUtils.showToast(ShuangYSaveMoneyActivity.this, "-开卡提交-" + status);
    ...
    // // 提交支付记录
    private void UpAlipayRrecord(int a, int b) {
        Call<Bean_BaseCard> bean_baseCardCall = HttpManager.getInstance().getHttpClient3().rechargeRecordSave(
                mOut_trade_no, mTrade_no, mMCardid, a, ShareUtil.getString(Contants.LOGIN_USER_PHONE),
                "", ShareUtil.getString(Contants.LOGIN_USERMEMBERID),
                money, b, 0, mPay_time != null ? mPay_time : "", mSubject != null ? mSubject : "");
        bean_baseCardCall.enqueue(new Callback<Bean_BaseCard>() {
            @Override
            public void onResponse(Call<Bean_BaseCard> call, Response<Bean_BaseCard> response) {
    ...
    BaseCallback mlistener = new BaseCallback() {
        ...
        public void callBack(BaseResp result) {
            //result 支付结果回调
            Log.e(TAG, " mlistener  支付回调结果  trace_id: " + result.trace_id);
            Log.e(TAG, " mlistener  支付回调结果  message: " + result.message);
            Log.e(TAG, " mlistener  支付回调结果  biz_content: " + result.biz_content);
            Log.e(TAG, " mlistener  支付回调结果  server_time: " + result.server_time);
            Log.e(TAG, " mlistener  支付回调结果  sign: " + result.sign);
            Log.e(TAG, " mlistener  支付回调结果  code: " + result.code);
            Log.e(TAG, " mlistener  支付回调结果  biz_type: " + result.biz_type);
            Log.e(TAG, " mlistener  支付回调结果  data: " + result.data);
            Log.e(TAG, " mlistener  支付回调结果  token: " + result.token);
            Log.e(TAG, " mlistener  支付回调结果  sign_type: " + result.sign_type);
            if (result.code == 5001) {
//                finish();
                ToastUtils.showToast(ShuangYSaveMoneyActivity.this, result.message);
            }
        }
    };
