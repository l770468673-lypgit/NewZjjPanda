package com.zjjxl.zjjxl_drawLayout_panda.drawlay.logins;
...
import com.zjjxl.zjjxl_drawLayout_panda.drawlay.Main_DrawerLayout;
import com.zjjxl.zjjxl_drawLayout_panda.https.ClientRestAPI;
import com.zjjxl.zjjxl_drawLayout_panda.https.HttpManager;
import com.zjjxl.zjjxl_drawLayout_panda.interceptor.AddCookiesInterceptor;
import com.zjjxl.zjjxl_drawLayout_panda.interceptor.ReceivedCookiesInterceptor;
...
public class ForgetPassActivity extends AppCompatActivity implements View.OnClickListener, TextWatcher {
    ...
    private void resetPassord() {
        ...
        LUtils.d(TAG,"getUserMid==========="+ mUserInfo.getUserMid());
        Call<queryUserInfoByUserMobileBean> bindCardIsBeanCall = HttpManager.getInstance().getHttpClient3()
                .resetUserPassword(mUserInfo.getUserMid(), newpass);
        bindCardIsBeanCall.enqueue(new Callback<queryUserInfoByUserMobileBean>() {
            @Override
            public void onResponse(Call<queryUserInfoByUserMobileBean> call,
                                   Response<queryUserInfoByUserMobileBean> response) {
                if (response.body() != null) {
                    if (response.body().isStatus()) {
                        ...
                        ShareUtil.putString(Contants.LOGIN_ORGMEMBERID, mUserInfo.getOrgMid());
                        ShareUtil.putString(Contants.LOGIN_USERMEMBERID, mUserInfo.getUserMid());
                        finish();
            ...
            @Override
            public void onFailure(Call<queryUserInfoByUserMobileBean> call, Throwable t) {
    ...
    private void getSmsCode(String mPhoneNum) {
        ...
        mClientRestAPIsend.sendVerificationCode(mPhoneNum, "XMK").enqueue(new Callback<SmsCode>() {
            @Override
            public void onResponse(Call<SmsCode> call, Response<SmsCode> response) {
                if (response.body() != null) {
            ...
            bindCardIsBeanCall.enqueue(new Callback<BindCardIsBean>() {
                ...
                public void onResponse(Call<BindCardIsBean> call, Response<BindCardIsBean> response) {
                    if (response.body() != null) {
                        ...
                        if (status) {
                            ToastUtils.showToast(ForgetPassActivity.this, response.body().getMsg() + "");
                            Intent intent = new Intent(ForgetPassActivity.this, Main_DrawerLayout.class);
            ...
            public void onResponse(Call<QueryUserInfo> call, Response<QueryUserInfo> response) {
                if (response.body() != null) {
                    ...
                    if (status) {
                        ...
                        ToastUtils.showToast(ForgetPassActivity.this, "该手机号码不存在,请先注册");
